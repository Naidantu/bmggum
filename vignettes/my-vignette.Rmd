...
# Obtain the model fit statistic adjusted chi-square degrees of freedom ratios
#>chisq.df <- modfit.bmggum(mod, index='chisq.df')
#>chisq.df
```

The function modfit.bmggum() provides model fit statistics. The elpd_waic and elpd_loo in the output indicate goodness of fit, meaning the larger elpd_waic and elpd_loo are, the better model fit. The waic and looic in the output indicate badness of fit, meaning the smaller waic and looic are, the better model fit. See documentation for the package **loo**, **GGUM**, Vehtari, Gelman, & Gabry (2017), Drasgow et al.(1995), and Tendeiro & Castro-Alvarez (2019) for more details.

* __index__. Model fit indices. The default is loo, which is the leave-one-out cross-validation. It can also be "waic", which is the widely applicable information criterion, or "chisq.df", which is the adjusted chi-square degrees of freedom ratios calculated for each trait separately that were introduced by Drasgow et al.(1995). Note that chisq.df can only be computed when the sample size is large.
...
```{r}
# Obtain auto-correlation plots for estimates of trait 1 and trait 2 for the first person (theta[1,1] and theta[1,2]).
#>bayesplot.bmggum(x=mod, pars=paste0("theta[1,",1:2,"]"), plot='autocorrelation', inc_warmup=F)
```
